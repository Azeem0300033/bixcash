...
class UserController extends Controller
{
    public function __construct()
        ...
        public function paymentdatatables()
        {
             $datas = UserPaymentMaster::orderBy('id')->get();
             //--- Integrating This Collection Into Datatables
             return Datatables::of($datas)
                                ->addColumn('action', function(UserPaymentMaster $data) {
                                     $class = $data->pay_status == 1 ? 'drop-success' : 'drop-danger';
                                    $s = $data->pay_status == 0 ? 'selected' : '';
                                    $ns = $data->pay_status == 1 ? 'selected' : '';
                                    $ban = '<select class="process select droplinks '.$class.'">'.
                '<option data-val="0" '.$s.'>Unpaid</option>'.
                '<option data-val="1" '.$ns.'>Paid</option></select>';
        ...
        {
            return view('admin.user.paymentview');
        }
        ...
        {   
            return view('admin.user.treeview',compact('data'));
        }
        
        
        public function treedatatable()
        {  
            if(!empty($_GET['level'])){
            $datas = User::where('level','=',$_GET['level'])->orderBy('id')->get();    
            }else{
            $datas = User::where('level','=',1)->orderBy('id')->get();
            }
             //--- Integrating This Collection Into Datatables
             return Datatables::of($datas)
                                ->addColumn('action', function(User $data) {
                                    ...
                                    return '<div class="action-list"><a href="' . route('admin-user-show',$data->id) . '" > <i class="fas fa-eye"></i> Details</a></div>';
                                }) 
                                ->rawColumns(['action'])
                                ->toJson();
        }

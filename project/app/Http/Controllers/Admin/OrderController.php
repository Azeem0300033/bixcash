...
class OrderController extends Controller
{
   ...
   public function payments($id,$amt)
    {  
        $toperamt =  ($amt*55)/100;
        
        $datas = User::where('id','<=',$id)->where('is_vendor','=',0)->orderBy('id')->get(); 
        
        $disamt = $toperamt/count($datas);
        if(!empty($datas)){
            foreach($datas as $key => $val){
        $data = new UserPaymentMaster;
        $data->user_id = $val->id;
    ...
    public function update(Request $request, $id)
    {
        ...
        //--- Redirect Section Ends     
            ...
            if ($input['status'] == "completed"){
                ...
                $this->payments($data->user_id,$data->pay_amount);
            }
